	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if ((nome.isEmpty() || nome.equals("") || nome.equals(" ") || nome.isBlank())) {
			RequestDispatcher dispatcher = request
					.getRequestDispatcher("/WEB-INF/view/usuario_cadastro_falha_nome.jsp");
			dispatcher.forward(request, response);
		} else if ((login.isEmpty() || login.equals("") || login.equals(" ") || login.isBlank())) {
			RequestDispatcher dispatcher = request
					.getRequestDispatcher("/WEB-INF/view/usuario_cadastro_falha_login.jsp");
			dispatcher.forward(request, response);
		} else if ((password.isEmpty() || password.equals("") || password.equals(" ") || password.isBlank())) {
			RequestDispatcher dispatcher = request
					.getRequestDispatcher("/WEB-INF/view/usuario_cadastro_falha_senha.jsp");
			dispatcher.forward(request, response);
		} else {
			...
			try {
				if (udao.verificarLogin(login)) {
					RequestDispatcher dispatcher = request
							.getRequestDispatcher("/WEB-INF/view/usuario_cadastro_login_existente.jsp");
					dispatcher.forward(request, response);
				} else {
					...
					RequestDispatcher dispatcher = request
							.getRequestDispatcher("/WEB-INF/view/usuario_cadastro_sucesso.jsp");
					dispatcher.forward(request, response);
				}
			} catch (ClassNotFoundException e) {
				e.printStackTrace();
